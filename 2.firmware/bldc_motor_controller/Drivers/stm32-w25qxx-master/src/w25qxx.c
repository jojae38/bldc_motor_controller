...
#ifdef W25QXX_QSPI
if (HAL_QSPI_Transmit(w25qxx->qspiHandle, buf, HAL_MAX_DELAY) == HAL_OK) {
    ret = W25QXX_Ok;
}
#else
...
W25QXX_result_t w25qxx_receive(W25QXX_HandleTypeDef *w25qxx, uint8_t *buf, uint32_t len) {
    W25QXX_result_t ret = W25QXX_Err;
#ifdef W25QXX_QSPI
  if (HAL_QSPI_Receive(w25qxx->qspiHandle, buf, HAL_MAX_DELAY) == HAL_OK) {
      ret = W25QXX_Ok;
  }
#else
...
#ifdef W25QXX_QSPI
W25QXX_result_t w25qxx_init(W25QXX_HandleTypeDef *w25qxx, QSPI_HandleTypeDef *hqspi, GPIO_TypeDef *cs_port, uint16_t cs_pin) {
  W25QXX_result_t result = W25QXX_Ok;

  W25_DBG("w25qxx_init");

  w25qxx->qspiHandle = hqspi;
  ...
  if (id) {
      ...
      case W25QXX_MANUFACTURER_WINBOND:
          ...
          switch (w25qxx->device_id) {
          case 0x4017:
              w25qxx->block_count = 0x80; //8MB
              break;
          case 0x4018:
              w25qxx->block_count = 0x100; //16MB
              break;
          case 0x4019:
              w25qxx->block_count = 0x200; //16MB
              break;
          default:
